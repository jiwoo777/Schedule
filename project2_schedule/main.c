...
int main(int argc, char *argv[]) {
	...
	//fill code here ----
	printf("input the name, place, typeName : ");
	scanf("%s %s, %s", name, place, typeName);
	
	if((fp = fopen(name, "r")) == NULL)
	{
		printf("input name is invalid(%s)\n, name");
		return -1;
	}
	
	if((fp = fopen(place, "r")) == NULL)
	{
		printf("input place is invalid(%s)\n, place");
		return -1;
	}
	
	if((fp = fopen(name, "r")) == NULL)
	{
		printf("input typeName is invalid(%s)\n, typeName");
		return -1;
	}
